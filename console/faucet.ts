...
(async () => {
    ...
    await connection.confirmTransaction(
        await connection.requestAirdrop(oracle, LAMPORTS_PER_SOL *2),
        "processed"
    );
    await connection.confirmTransaction(
    ...
    await connection.confirmTransaction(
        await connection.requestAirdrop(bob, LAMPORTS_PER_SOL *2),
        "processed"
    );
    await connection.confirmTransaction(
        await connection.requestAirdrop(alice, LAMPORTS_PER_SOL *2),
        "processed"
    );
    await connection.confirmTransaction(
        await connection.requestAirdrop(cha, LAMPORTS_PER_SOL *2),
        "processed"
    );
